...
class DataHandlerLSTM:
    ...
    def __init__(self, args):
        ...
        # Training variables
        self.sequence_reset = np.ones([
            self.batch_size])  # indicates whether sequences are reset (hidden state of LSTMs needs to be reset accordingly)
        ...
        self.batch_x = np.zeros(
            [self.batch_size, self.tbpl, self.input_dim * (self.prev_horizon + 1)])  # data fed for training
        ...
        if self.others_info == "relative":
            self.pedestrian_grid = np.zeros(
                [self.batch_size, self.tbpl, self.pedestrian_vector_dim * self.n_other_agents])
            self.val_pedestrian_grid = np.zeros(
                [self.batch_size, self.tbpl, self.pedestrian_vector_dim * self.n_other_agents])
        ...
        self.val_sequence_reset = np.ones([
            self.batch_size])  # indicates whether sequences are reset (hidden state of LSTMs needs to be reset accordingly)
    ...
    def describe(self):
        ...
        print("Providing Summary of DataHandler Attributes:\n")
        for k, v in vars(self).items():
            if isinstance(v, np.ndarray):
                v = f"numpy array: {v.shape}"
            if k == "trajectory_set":
                v = f"list[(int, src.data_utils.Trajectory.Trajectory)], len: {len(v)}"
    ...
    def compute_min_max_values(self):
        for traj_id in range(len(self.trajectory_set)):
            for t_id in range(1, self.trajectory_set[traj_id][1].pose_vec.shape[0]):
                self.trajectory_set[traj_id][1].vel_vec[t_id, :2] = (self.trajectory_set[traj_id][1].pose_vec[t_id,
                                                                     :2] -
    ...
    def _process_real_data_(self):
        ...
        self.agent_container.occupancy_grid.gridmap = np.zeros(
            [int(self.agent_container.occupancy_grid.map_size[0] / self.agent_container.occupancy_grid.resolution),
             int(self.agent_container.occupancy_grid.map_size[
                     1] / self.agent_container.occupancy_grid.resolution)])  # occupancy values of cells
        self.agent_container.occupancy_grid.center = self.agent_container.occupancy_grid.map_size / 2.0
        ...
        H = np.genfromtxt(os.path.join(self.data_path + self.scenario, 'H.txt'), delimiter='  ',
        ...
        for obst_ii in range(self.obstacles_2d.shape[0]):
            obst_idx = self.agent_container.occupancy_grid.getIdx(self.obstacles_2d[obst_ii, 0],
                                                                  self.obstacles_2d[obst_ii, 1])
            self.agent_container.occupancy_grid.gridmap[obst_idx] = 1.0

        # fig = plt.figure("Global Trajectory Predictions")
        # ax_in = plt.subplot()
        # ax_in.clear()
        # # plot scenario grid
        ...
        if os.path.exists(self.data_path + self.scenario + '/obsmat.txt'):
            pedestrian_data = np.genfromtxt(os.path.join(self.data_path + self.scenario, 'obsmat.txt'), delimiter="  ")[
                              1:, :]        # why skip the first row?
            pixel_data = False
        elif os.path.exists(self.data_path + self.scenario + '/obsmat_px.txt'):
            pedestrian_data = np.genfromtxt(os.path.join(self.data_path + self.scenario, 'obsmat_px.txt'),
                                            delimiter="  ")[1:, :]
            pixel_data = True
        else:
            print(colorama.Fore.RED + "Could not find obsmat.txt or obsmat_px.txt" + colorama.Style.RESET_ALL)
            sys.exit()
        ...
        for sample_idx in range(pedestrian_data.shape[0]):
            ...
            if pixel_data:
                converted_pose = sup.to_pos_frame(H, np.expand_dims(
                    np.array((pedestrian_data[sample_idx, idx_posx], pedestrian_data[sample_idx, idx_posy])),
                    axis=0).astype(float))
        ...
        # THIS FUNCTION ALSO UPDATES VELOCITY VALUES
        self.compute_min_max_values()
    ...
    def getAgentTrajectory(self, agent_id):
        ...
        self.agent_traj_idx[agent_id] = (self.agent_traj_idx[
                                             agent_id] + 1) % self.agent_container.getNumberOfTrajectoriesForAgent(
            agent_id)
        return trajectory
    ...
    def fillBatch(self, agent_id, batch_idx, start_idx, truncated_backprop_length, batch_x, batch_vel, batch_pos,
                  batch_grid, pedestrian_grid, batch_goal, batch_y, trajectory, batch_pos_target, centered_grid=False,
                  testing=False):
        """
        ...
        for tbp_step in range(truncated_backprop_length):
            ...
            for prev_step in range(self.prev_horizon, -1, -1):
                ...
                current_vel = np.array(
                    [trajectory.vel_vec[start_idx + tbp_step - prev_step, 0],
                     trajectory.vel_vec[start_idx + tbp_step - prev_step, 1]]
                )
                ...
                batch_x[batch_idx, tbp_step, prev_step * self.input_dim:(prev_step + 1) * self.input_dim] = np.array(
                    [current_pos[0],
                     current_pos[1],
                     current_vel[0],
                     current_vel[1]]
                ...
                prev_step * self.input_state_dim:(prev_step + 1) * self.input_state_dim] = np.array([current_vel[0],
                                                                                                     current_vel[1]])
                batch_pos[batch_idx, tbp_step,
                prev_step * self.input_state_dim:(prev_step + 1) * self.input_state_dim] = np.array([current_pos[0],
                                                                                                     current_pos[1]])
            ...
            current_pos = np.array(
                [trajectory.pose_vec[start_idx + tbp_step, 0],
                 trajectory.pose_vec[start_idx + tbp_step, 1]]
            )
            current_vel = np.array(
                [trajectory.vel_vec[start_idx + tbp_step, 0],
                 trajectory.vel_vec[start_idx + tbp_step, 1]]
            )
            heading = math.atan2(current_vel[1], current_vel[0])
            ...
            if self.others_info == "relative":
                for ag_id in range(min(n_other_agents, self.n_other_agents)):
                    rel_pos = np.array([other_poses_ordered[ag_id, 0] - current_pos[0],
                                        other_poses_ordered[ag_id, 1] - current_pos[1]]) * \
                              multivariate_normal.pdf(
                                  np.linalg.norm(np.array([other_poses_ordered[ag_id, :2] - current_pos])),
                                  mean=0.0, cov=5.0)
                    rel_vel = np.array([other_poses_ordered[ag_id, 2] - current_vel[0],
                                        other_poses_ordered[ag_id, 3] - current_vel[1]])
                ...
                radial_pedestrian_grid = sup.compute_radial_distance_vector(self.pedestrian_vector_dim,
                                                                            ...
                                                                            max_range=self.max_range_ped_grid,
            ...
            for pred_step in range(self.output_sequence_length):
                ...
                if self.normalize_data:
                    self.normalize_vel(batch_y[batch_idx, tbp_step,
                                       self.output_state_dim * pred_step:self.output_state_dim * pred_step + 2])
                batch_pos_target[batch_idx, tbp_step, self.output_state_dim * pred_step] = px - trajectory.pose_vec[
                    start_idx + tbp_step, 0]
                batch_pos_target[batch_idx, tbp_step, self.output_state_dim * pred_step + 1] = py - trajectory.pose_vec[
                    start_idx + tbp_step, 1]
    ...
    def getBatch(self):
        ...
        return deepcopy(self.batch_x), deepcopy(self.batch_vel), deepcopy(self.batch_pos), deepcopy(self.batch_goal), \
               deepcopy(self.batch_grid), deepcopy(self.pedestrian_grid), deepcopy(self.batch_y), deepcopy(
            self.batch_pos_target), \
    ...
    def getTestBatch(self):
        ...
        for ii, traj in enumerate(self.val_batch_sequences):
            if self.val_sequence_idx[ii] + self.tbpl + self.output_sequence_length + 1 >= len(traj):
                id, trajectory = self.trajectory_set[self.val_data_idx]
                self.val_data_idx = (self.val_data_idx + 1) % int(
                    len(self.trajectory_set) * (1 - self.train_set) - 1) + int(
                    len(self.trajectory_set) * self.train_set)
        ...
        for ii in range(0, min(self.batch_size, len(self.trajectory_set) - len(self.trajectory_set) % self.batch_size)):
            ...
            other_agents_pos.append(
                self.fillBatch(agent_id, ii, int(self.val_sequence_idx[ii]), self.tbpl, self.val_batch_x,
                               self.val_batch_vel, self.val_batch_pos, self.val_batch_grid, self.val_pedestrian_grid,
                               self.val_batch_goal,
        ...
        if self.rotated_grid:
            ...
            self.val_batch_x, self.val_batch_pos_target = sup.rotate_batch_to_local_frame(self.val_batch_pos_target,
                                                                                          self.val_batch_x)
    ...
    def getTrajectoryAsBatch(self, trajectory_idx, max_sequence_length=1000, freeze=False):
        ...
        sequence_length = min(max_sequence_length,
                              traj.pose_vec.shape[0] - self.output_sequence_length - self.prev_horizon)
        ...
        if self.others_info == "relative":
            ...
            pedestrian_grid = np.zeros([1, sequence_length,
                                        int(np.ceil(
                                            self.submap_width / self.agent_container.occupancy_grid.resolution)),
                                        int(np.ceil(
                                            self.submap_height / self.agent_container.occupancy_grid.resolution))])
        else:
            pedestrian_grid = np.zeros([1, sequence_length, self.pedestrian_vector_dim])
        other_agents_pos = self.fillBatch(id, 0, self.prev_horizon, sequence_length, batch_x, batch_vel, batch_pos,
                                          batch_grid, pedestrian_grid, batch_goal, batch_y, traj, batch_pos_target,
    ...
    def getGroupOfTrajectoriesAsBatch(self, trajectory_idx, max_sequence_length=1000, n_trajs=1):
        ...
        other_agents_pos_list.append(
            self.fillBatch(agent_id, 0, self.prev_horizon, sequence_length, batch_x, batch_vel, batch_pos,
        ...
        for id in self.agent_container.agent_data.keys():
            if id != agent_id:
                ...
                pedestrian_grid = np.zeros(
                    [1, sequence_length, self.n_other_agents, self.pedestrian_vector_dim * self.prediction_horizon])
                other_agents_pos_list.append(
                    self.fillBatch(id, 0, self.prev_horizon, sequence_length, batch_x, batch_vel, batch_pos, batch_grid,
                                   pedestrian_grid, batch_goal, batch_y, t, batch_pos_final,
                                   centered_grid=self.centered_grid))
                batch_x_list.append(batch_x)
                batch_vel_list.append(batch_vel)
                batch_pos_list.append(batch_pos)
    ...
    def getTestTrajectoryAsBatch(self, trajectory_idx, max_sequence_length=1000, freeze=False):
        ...
        other_agents_pos = self.fillBatch(id, 0, self.prev_horizon, sequence_length, batch_x, batch_vel, batch_pos,
                                          batch_grid, pedestrian_grid, batch_goal, batch_y, traj, batch_pos_final,
                                          centered_grid=self.centered_grid, testing=True)
    ...
    def getAgentDataAtTimestep(self, query_time):
        ...
        for id in self.agent_container.agent_data.keys():
            ...
            if traj_for_time != None:
                ...
                grid = self.agent_container.occupancy_grid.getSubmapByCoords(pose[0], pose[1], self.submap_width,
                                                                             self.submap_height)
                rotated_grid = sup.rotate_grid_around_center(grid, -heading * 180 / math.pi)
                radial_pedestrian_grid = np.zeros(
                    [self.pedestrian_vector_dim])  # Initialize pedestriangrid with 0, will be evaluated later
        ...
        for id in agent_dict.keys():
            ...
            for other_id in agent_dict.keys():
                if other_id != id:
                    other_agents_pos = np.append(other_agents_pos, np.expand_dims(agent_dict[other_id][0][:2], axis=0),
                                                 axis=0)
    ...
    def plot_global_scenario(self, batch_grid, batch_x, batch_agent_real_traj, batch_goal, other_agents_pos,
                             batch_agent_predicted_traj, t, seq_index, block=False):
        """
			inputs:
        ...
        c_norm = plt.matplotlib.colors.Normalize(vmin=0, vmax=100)
        scalar_color_map = plt.cm.ScalarMappable(norm=c_norm, cmap=colormap)
        ...
        if batch_agent_predicted_traj is not None:
            ...
            for mix_idx in range(self.n_mixtures):
                ...
                traj_pred = sup.path_from_vel(initial_pos=np.array(
                    [batch_x[seq_index, t, 0] * self.norm_const_x, batch_x[seq_index, t, 1] * self.norm_const_y]),
                    pred_vel=vel_pred, dt=self.dt, n_vx=self.norm_const_vx,
                    n_vy=self.norm_const_vy)
        ...
        real_traj_global_frame = sup.path_from_vel(initial_pos=np.array(
            [batch_x[seq_index, t, 0] * self.norm_const_x, batch_x[seq_index, t, 1] * self.norm_const_y]),
            pred_vel=real_traj, dt=self.dt, n_vx=self.norm_const_vx,
            n_vy=self.norm_const_vy)
    ...
    def plot_local_scenario(self, batch_grid, batch_x, batch_agent_real_traj, batch_goal, other_agents_pos,
                            batch_agent_predicted_traj, t, seq_index, block=False):
        """
			inputs:
        ...
        fig = plt.figure("Local Trajectory Predictions")
        ax_out = plt.subplot()
        ...
        if self.multi_pedestrian:
            ...
            try:
                ax_out.plot(other_pos[0, 0] - x_init_global_frame, other_pos[0, 1] - y_init_global_frame, color='r',
                            marker='o', label='Other agents')
                for jj in range(1, other_pos.shape[0]):
                    ax_out.plot(other_pos[jj, 0] - x_init_global_frame, other_pos[jj, 1] - y_init_global_frame,
                                color='r', marker='o')  # ,markersize=23)  # other agent pos positon
                # ax_out.annotate(str(jj), xy=(other_pos[jj, 0]-x_init_global_frame , other_pos[jj, 1]-y_init_global_frame))
            except IndexError:
                print("Oops!  That was no valid number.  Try again...")

        circle = plt.Circle((0, 0), self.max_range_ped_grid, color='blue',
							fill=False)
        ...
        for seq_index in range(batch_x.shape[0]):
            for t in range(batch_x.shape[1]):
                ...
                for i in range(real_traj_global_frame.shape[0]):
                    x = int(max(0, int(np.abs(real_traj_global_frame[
                                                  i, 0] + self.submap_width / 2) / self.agent_container.occupancy_grid.resolution)))
                    y = int(max(0, int(np.abs(real_traj_global_frame[
                                                  i, 1] + self.submap_height / 2) / self.agent_container.occupancy_grid.resolution)))
                    batch_grid[seq_index, t, x, y] = 1

    def plot_angular_grid(self, batch_ped_grid, t, seq_index):
        if self.multi_pedestrian:
            fig = plt.figure("Angular grid")
            ...
            ax_ped_grid.arrow(0, 0, 1, 0, head_width=0.1,
                              head_length=self.max_range_ped_grid)  # agent poiting direction
    ...
    def query_agent_velocity_direction(self, batch_grid, batch_x, batch_agent_real_traj, other_agents_pos, t,
                                       seq_index):
        """
			inputs:
				global_grid: map grid of the scenario
        ...
        # pl.close('all')
        global_grid = self.agent_container.occupancy_grid
        fig = plt.figure("Query agent direction")
        ax_in = plt.subplot()
        ...
        ax_in.arrow(x_init_global_frame, y_init_global_frame, batch_x[seq_index, t, 2] * self.norm_const_vx,
                    batch_x[seq_index, t, 3] * self.norm_const_vy
                    , head_width=0.1, head_length=self.max_range_ped_grid + 0.5, alpha=0.4,
                    label='Agent velocity direction')  # angular grid detection distance
        circle = plt.Circle((x_init_global_frame, y_init_global_frame), self.max_range_ped_grid, color='blue',
							fill=False, label='Detection range')
