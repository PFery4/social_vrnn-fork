        ...
        with open("trained_models_summary.csv", 'w') as csvfile:
            ...
            if len(lines) > args.exp_num + 1:
                for i in range(len(lines)):
                    if i == args.exp_num + 1:
                        writer.writerow(
                            [args.model_name + "_" + str(args.exp_num), args.scenario, args.pedestrian_vector_dim,
                             args.batch_size,
                             training_loss, 0, 0, args.n_epochs,
                             args.learning_rate_init, args.grads_clip, args.n_mixtures])
                    else:
...
def write_results_summary(mse_dict, fde_dict, avg_div_list, args, test_args):
    file_name = f"{test_args.model_name}_summary.csv"

    title_row = ["Model name", "Dataset", "Ped Info Size",
                 "Batch size", "dt", "Prediction Horizon",
                 "Previous Steps", "tbpt", "Test Dataset",
                 "Others Info", "min_MSE", "min_FDE",
                 "mean_MSE", "mean_FDE",
                 "max_MSE", "max_FDE", "Diversity"]

    new_row = [f"{args.model_name}_{args.exp_num}", args.scenario, args.pedestrian_vector_dim,
               args.batch_size, args.dt, args.prediction_horizon,
               args.prev_horizon, args.truncated_backprop_length, test_args.scenario,
               args.others_info, np.mean(mse_dict["avg_min_mse_list"]), np.mean(fde_dict["avg_min_fde_list"]),
               np.mean(mse_dict["avg_mean_mse_list"]), np.mean(fde_dict["avg_mean_fde_list"]),
               np.mean(mse_dict["avg_max_mse_list"]), np.mean(fde_dict["avg_max_fde_list"]), np.mean(avg_div_list)]
            ...
            if len(lines) > 3000:  # args.exp_num + 1:
                for i in range(len(lines)):
                    if i == args.exp_num + 1:
                        writer.writerow(
                            [args.model_name + "_" + str(args.exp_num), args.scenario, args.pedestrian_vector_dim,
                             args.batch_size, args.n_steps, loss])
                    else:
                        writer.writerow(lines[i])
            else:
                writer.writerows(lines)
                writer.writerow([args.model_name + "_" + str(args.exp_num), args.scenario, args.pedestrian_vector_dim,
                                 args.batch_size, args.n_steps, loss])
...
def parse_args(defaults, parse_type):
    ...
    parser.add_argument('--model_name', help='Path to directory that comprises the model (default="model_name").',
                        type=str, default=defaults['model_name'])
    parser.add_argument('--exp_num', help='Experiment number', type=int, default=defaults['exp_num'])
    parser.add_argument('--scenario', help='Scenario of the dataset (default="").', type=str,
    ...
    if parse_type == "Train" or parse_type == "train":
        parser.add_argument('--input1_type', help='Data type for the first input', type=str,
                            default=defaults['input1_type'])
        parser.add_argument('--input2_type', help='Data type for the second input', type=str,
                            default=defaults['input2_type'])
        parser.add_argument('--model_path',
                            help='Path to directory to save the model (default=""../trained_models/"+model_name").',
                            type=str, default=defaults['model_path'])
        parser.add_argument('--pretrained_convnet_path',
                            help='Path to directory that comprises the pre-trained convnet model (default=" ").',
                            type=str, default=defaults['pretrained_convnet_path'])
        ...
        parser.add_argument('--real_world_data', help='Real world dataset (default=True).', type=sup.str2bool,
                            default=defaults['real_world_data'])
        parser.add_argument('--dataset', help='Dataset pkl file', type=str, default=defaults['scenario'] + '.pkl')
        parser.add_argument('--dataset_val', help='Dataset pkl file', type=str,
                            default=defaults['scenario_val'] + '.pkl')
        ...
        parser.add_argument('--warmstart_model', help='Restore from pretained model (default=False).', type=bool,
                            default=defaults['warmstart_model'])
        parser.add_argument('--warm_start_convnet', help='Restore from pretained convnet model (default=False).',
                            type=bool, default=defaults['warm_start_convnet'])
        parser.add_argument('--dt', help='Data samplig time (default=0.3).', type=float, default=defaults['dt'])
        parser.add_argument('--n_steps', help='Number of epochs (default=10000).', type=int,
        ...
        parser.add_argument('--regularization_weight', help='Weight scaling of regularizer (default=0.01).', type=float,
                            default=defaults['regularization_weight'])
        ...
        parser.add_argument('--rnn_state_size', help='Number of RNN / LSTM units (default=16).', type=int,
                            default=defaults['rnn_state_size'])
        parser.add_argument('--rnn_state_size_lstm_ped',
                            help='Number of RNN / LSTM units of the pedestrian lstm layer (default=32).', type=int,
                            default=defaults['rnn_state_size_lstm_ped'])
        parser.add_argument('--rnn_state_size_bilstm_ped',
                            ...
                            type=int, default=defaults['rnn_state_size_bilstm_ped'])
        parser.add_argument('--rnn_state_size_lstm_grid',
                            help='Number of RNN / LSTM units of the grid lstm layer (default=32).', type=int,
                            default=defaults['rnn_state_size_lstm_grid'])
        parser.add_argument('--rnn_state_size_lstm_concat',
                            help='Number of RNN / LSTM units of the concatenation lstm layer (default=32).', type=int,
                            default=defaults['rnn_state_size_lstm_concat'])
        parser.add_argument('--prior_size', help='prior_size', type=int, default=defaults['prior_size'])
        parser.add_argument('--latent_space_size', help='latent_space_size', type=int,
                            default=defaults['latent_space_size'])
        parser.add_argument('--x_dim', help='x_dim', type=int, default=defaults['x_dim'])
        parser.add_argument('--fc_hidden_unit_size',
                            help='Number of fully connected layer units after LSTM layer (default=64).', type=int,
                            default=defaults['fc_hidden_unit_size'])
        parser.add_argument('--input_state_dim', help='Input state dimension (default=).', type=int,
                            default=defaults['input_state_dim'])
        ...
        parser.add_argument('--print_freq', help='Print frequency of training info (default=100).', type=int,
                            default=defaults['print_freq'])
        parser.add_argument('--save_freq', help='Save frequency of the temporary model during training. (default=20k).',
                            type=int, default=defaults['save_freq'])
        parser.add_argument('--patience', help='Patience to stop the training loop early', type=int,
        ...
        parser.add_argument('--predict_positions', help='predict_positions.', type=sup.str2bool,
                            default=defaults['predict_positions'])
