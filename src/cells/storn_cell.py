...
class VariationalRNNCell(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self,args, x_dim, y_dim,h_dim ,z_dim = 100):
        ...
        self.lstm = tf.contrib.rnn.LSTMCell(self.n_dec_hidden, state_is_tuple=True,reuse=tf.AUTO_REUSE)
        self.args = args
    ...
    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            #   Eq 5
            with tf.variable_scope("Storn_Prior"):
            ...
            with tf.variable_scope("Storn_phi_x"):
                x_1 = tf.nn.relu(linear(x, self.n_x_1))
                #x_1 = linear(x, self.n_x_1)

            with tf.variable_scope("Storn_Encoder"):
                with tf.variable_scope("hidden"):
            ...
            eps = tf.random_normal(tf.shape(enc_mu), 0.0, 1.0, dtype=tf.float32)
            # z = mu + sigma*epsilon
            z = tf.add(enc_mu, tf.multiply(enc_sigma, eps))
            with tf.variable_scope("Storn_phi_z"):
                z_1 = tf.nn.relu(linear(z, self.n_z_1))

            # propagate hidden state eq(7)
            concat = tf.concat(axis=1, values=(x_1, z_1))
            output, state2 = self.lstm(z_1, state)
